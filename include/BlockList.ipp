...
template<typename T>
SemiBlock<T>::SemiBlock(int _this_pos,FileOperator &fo) noexcept
{
    if(_this_pos!=-1&&_this_pos<fo.size())
    {
        fo.read(_this_pos,this);
        if(siz!=0) fo.read(_this_pos+4*sizeof(int)+(siz-1)*T::memory_size,&max_e);
    }
    else
    {
...
// #endif
    Block<T> u(u_pos,fo);
// #ifdef DEBUG
//     std::cout<<"Node #"<<u_pos<<" 's size = "<<u.size()<<std::endl;
// #endif
